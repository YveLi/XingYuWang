...
{
    public class CartoonCommentService : BaseService<CartoonComment, int>
    {
        /// <summary>
        /// 获取评论信息
        /// </summary>
        ...
        /// <returns></returns>
        public Response AddComment(CartoonComment entity, int currentUserId, IList<string> names = null)
        {
            var post = DbBase.SingleOrDefaultById<StarCartoon>(entity.ArticleId);//获取帖子信息 
            //如果不是自己回复自己，才添加一条消息
            if (post.TUserId != currentUserId)
            {
                var message = new Message
                {
                    ...
                    Href = "/StarCartoon/Detail/" + entity.ArticleId + "#articlecoment",
                    Content = post.Title,
            ...
            post.CommentCount++;
            //更新评论人数
            DbBase.UpdateMany<StarCartoon>().OnlyFields(p => p.CommentCount).Where(p => p.Id == entity.ArticleId).Execute(post);
