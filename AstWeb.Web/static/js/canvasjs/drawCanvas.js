...
// if('undefined' != typeof drawAstrolabeTest){return;}
(function (window) {
    ...
    **/
    ...
    var ctx = null;
    var configItem = [];
    var drawAstrolabeTest = function (_ctx, _width, _configItem, pType, style) {
        //设置画布
        var width = _width;
        ctx = _ctx;
        ctx.lineWidth = 2;
    ...
    window.drawAstrolabeTest = drawAstrolabeTest
})(window);
function drawXingPan({ canvas, width, panStyle, type, data1, data2, configItem }) {
    canvas.setAttribute('width', width*2);
    canvas.setAttribute('height', width*2);
    ...
    switch (type) {
        case 0: {//单盘
            draw = drawAstrolabeTest(ctx, width, configItem, '单盘', panStyle);
            draw(dealSingleData(data1.split(',')));
            break;
        } case 1: {//双盘
            draw = drawAstrolabeTest(ctx, width, configItem, '双盘', panStyle);
            draw(dealDoubleData(data1.split(','), data2.split(',')));
            break;
        } case 2: {//配对盘
            draw = drawAstrolabeTest(ctx, width, configItem, '单盘', panStyle);
            var objSynastry1 = dealSingleData(data1.split(','));
            var objSynastry2 = dealSingleData(data2.split(','));
            var objSynastry = {
            ...
            draw = drawAstrolabeTest(ctx, width, configItem, '法达', '专业');
            draw(dealSingleData(data1.split(',')));
            break;
        } default: {
            break;
